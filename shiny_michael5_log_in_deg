#setwd("C:/Users/michael/Desktop/Scotus_Network_2/scotus/with_node_attributes/shiny_michael5_log_in_deg")
...
shinyServer(function(input, output) {
  ...
  output$distPlot <- renderPlot({
    ...
    plot(log(as.integer(names(table(datasetInput())))), log(table(datasetInput())),
         main = paste('log-log plot of in-degree distribution of ', input$yearA),
         ...
         xlab = paste('log(in-degree) of ', input$yearA),
         ylab = 'log(counts)',
         col = "#FB8072"
    )
...
shinyUI(fluidPage(
  ...
  # Application title
  titlePanel("Log-Log Plot of In-Degree Distribution in Each Year of SCOTUS Network"),
