#setwd("C:/Users/michael/Desktop/Scotus_Network_2/scotus/with_node_attributes/shiny_michael5_cum_log_in_deg")
...
shinyServer(function(input, output) {
  ...
  output$distPlot <- renderPlot({
      ...
      plot(log(as.integer(names(table(datasetInput())))), log(table(datasetInput())),
           main = paste('log-log plot of cumulative in-degree distribution up to ', input$yearA),
           xlim = c(0, 7.2), #from log log plot of in-degree distribution of entire network and log(1295) == 7.166266
           ylim = c(0, 8.5), #from log log plot of in-degree distribution of entire network
           xlab = paste('cumulative log(in-degree) up to ', input$yearA), 
           ylab = 'log(counts)',
           col = "#FB8072"
      )
...
shinyUI(fluidPage(
  ...
  # Application title
  titlePanel("Log-Log Plot of Cumulative In-Degree Distribution Up to Some Year of SCOTUS Network"),
