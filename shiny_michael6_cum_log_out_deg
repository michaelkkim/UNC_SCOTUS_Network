#setwd("C:/Users/michael/Desktop/Scotus_Network_2/scotus/with_node_attributes/shiny_michael6_cum_log_out_deg")
...
shinyServer(function(input, output) {
  ...
  output$distPlot <- renderPlot({
    ...
    plot(log(as.integer(names(table(datasetInput())))), log(table(datasetInput())),
         main = paste('log-log plot of cumulative out-degree distribution up to ', input$yearA),
         ...
         xlab = paste('cumulative log(out-degree) up to ', input$yearA),
         ylab = 'log(counts)',
         col = "#39BEB1"
    )
...
shinyUI(fluidPage(
  ...
  # Application title
  titlePanel("Log-Log Plot of Cumulative Out-Degree Distribution Up to Some Year of SCOTUS Network"),
